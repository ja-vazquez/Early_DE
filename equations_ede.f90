...
!     rho_res = (grhob+grhoc)*a**(-3.0)
    ...
    real(dl), intent(IN) :: a
      Oede0 = CP%omegaede
      top = CP%omegav - Oede0*(1.0 - a**(-3.0*w_lam))
    end function top
    ...
    real(dl)  :: bot 
    real(dl), intent(IN) :: a
      bot = CP%omegav + a**(3.0+3*w_lam)*O_res(a) 
    end function bot
...
!if (a>1d-3) print *, a, O_ede
    ...
    real(dl) :: grho_ede
    real(dl), intent(IN) :: a
     grho_ede =  O_ede(a)*rho_res(a)/(1.0 - O_ede(a))

    end function grho_ede
    ...
    use ModelParams
    implicit none    
    real(dl) :: w_ede, dEDE, dO_res, dO_ede, dlnp_lna, Om0, Oede0
    real(dl), intent(IN) :: a
    Oede0 = CP%omegaede
    Om0 = 1 - CP%omegav  

    dEDE = 3.0*w_lam*Oede0*a**(-3.0*w_lam-1.0)
    dO_res = (-3.0*(grhoc+grhob)*a**(-4.0)-4.0*(grhog+grhornomass)*a**(-5.0))/grhom !-3.0*Om0*a**(-4.0) !-4.0*0.0001*a**(-5.0)
    dO_ede = (-dEDE*bot(a) - (3.0*(1.0+w_lam)*a**(3.0*(1.0+w_lam)-1.0)*O_res(a) \
                 + a**(3*(1.0+w_lam))*dO_res*top(a) ))/(bot(a)**2.0) + dEDE

    dlnp_lna = (a*dO_res*grhom/rho_res(a) + a*dO_ede/(O_ede(a)*(1.0 - O_ede(a))))   
    w_ede = -1.0 - dlnp_lna/3.0 
    end function w_ede    
...
!---------------------
    ...
    if(use_EDE) then
      if((1.0/a -1)<1.0e27) then 
        grho_de =grho_ede(a)*a**4.0
      else
...
!   dtauda =dtauda*(1.0 + CP%omegaede)**(-0.5) 
    end function dtauda
...
!JAV
!if(1.0/a-1.0 < 1000) print *,  1.0/a-1.0, adotoa/a
    ...
    !!!If we want DE perturbations to get \delta\rho/\rho_m
            !dgrho = dgrho+y(EV%w_ix)*grhov*a**(2.0)/a  !grhov*a**(-1-3*w_lam)
            !Arr(Transfer_r) = y(EV%w_ix)/k2
    
            !dgrho = dgrho+(clxc*grhoc + clxb*grhob)/a +y(EV%w_ix)*grhov*a**(2.0)
            !grho  =  grho+(grhoc+grhob)/a !+ grhov*a**(10.0) !grhov*a**(-1-3*w_lam)
