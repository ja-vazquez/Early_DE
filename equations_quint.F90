    ...
    ! set quintessence=T in .ini file to use quintessence rather than fluid model
    ...
    logical :: quintessence = .true.  !If true Om_Lambda is interpreted as quintessence, otherwise just Lambda
    ...
    !print *, '**Using Quintessence', quintessence
    contains
    ...
    !real(dl), parameter :: m = 100 !1E17  !5d-7
    !real(dl) :: sigma_model =  1.2247 !5 !1.2247
!    private, nothing
    contains
    ...
    !times (Mpc/c)^2 to get units in 1/Mpc^2
    !The input variable phi is sqrt(8*Pi*G)*psi
    use constants
  !  implicit none
    real(dl) phi,Vofphi
    integer deriv
    real(dl), parameter :: root8pi = 5.0132565492620010048_dl
    real(dl), parameter :: Tpl=5.391d-44*root8pi   !Tpl = sqrt(8 pi G hbar/c^5)
    real(dl) lambda, norm, Co, fact1, fact2, Al2_quin
    lambda = sigma_model
    ...
    !        if (deriv==0) then
    !          !norm multiplies kappa*V
    Vofphi= norm*exp(-lambda*phi)
    !         else if (deriv ==1) then
    !           Vofphi=-norm*lambda*exp(-lambda*phi)
    !         else if (deriv ==2) then
    !           Vofphi=norm*lambda**2*exp(-lambda*phi)
    !         else
    ...
    !     sigma_model= sqrt(3./2.)
    !B_quin= 6.0
    !A_quin= 0.01
    !V0 = 100
    !print *, 'A_qui', A_quin, quintessence , B_quin, V0, sigma_model 
norm = 1

    Co = 1.0
    Al2_quin = A_quin/sigma_model**2
    fact1= (Co*phi-B_quin)**2 + Al2_quin
...
!print *, 'Voi', Voi
    ...
    real(dl) a, a2, tot
    real(dl) phi,dphida, tmp, phidot, rhonu
    integer nu_i
...
Voi=dum
    ...
    tmp=a2*(0.5d0*phidot**2 + a2*Voi*Vofphi(phi,0))
    !if (a>0.9) print *, a, tmp
   
    tot=(grhoc+grhob)*a+grhog+grhornomass +grhok*a2 + tmp
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_rho(a*nu_masses(nu_i),rhonu)
            tot = tot +rhonu*grhormass(nu_i)
        end do
    end if
    ...
    dphida=phidot/adot
    yprime(1)=dphida
    yprime(2)= -a2**2*Voi*Vofphi(phi,1)/adot

!print *,'Voi2',  dum, Voi
!print *, 'params', A_quin, B_quin, sigma_model

    end subroutine EvolveBackground



    function Quint_GetOmegaFromInitial(astart,phi,phidot,atol, Voi)
...
!        print *,'Angular', 1.d0-1./aend, AngularDiameterDistance(1./aend - 1.d0)
        !print *, aend, real((0.5d0*phidot_a(i+1)**2 - Vofphi(phi_a(i+1),0))/(0.5d0*phidot_a(i+1)**2 + Vofphi(phi_a(i+1),0)))
!if (aend>0.0009 .and. aend<0.0011)print *,'**O_EDE', aend, (0.5d0*phidot_a(i+1)**2 +aend**2*Voi*Vofphi(phi_a(i+1),0))**aend**0/rhofrac

        if (i==NumPoints-1) then
            if (FeedbackLevel > 0) then
                write(*,*) 'Omega_Q_0=',real((0.5d0*phidot_a(i+1)**2 + Voi*Vofphi(phi_a(i+1),0))/(3*adot**2)), &
                '  w_0=',real((0.5d0*phidot_a(i+1)**2 - Voi*Vofphi(phi_a(i+1),0))/(0.5d0*phidot_a(i+1)**2 + &
                Voi*Vofphi(phi_a(i+1),0)))
                write (*,*) 'w_effective = ', real(effint/omint)
                write (*,*) 'Omega_lambda = ', CP%omegav
            end if
        end if
    end do
    close(50)
...
Voif = Voi
!DA = AngularDiameterDistance(0.40d0)
!print *,'Angular', AngularDiameterDistance(0.40d0)
...
!#######
    subroutine Quint_ValsAta(a,aphi,aphidot, Voip)
    implicit none
...
!print *,'Voi1113', Voif
    ...
    if (quintessence) call Quint_init_background
    ...
    if (quintessence) then
        call Quint_ValsAta(a,phi,phidot,Voip)
!print *, 'Voip', Voip
        grhoa2=grhoa2 + a2*(phidot**2/2 + a2*Voip*Vofphi(phi,0))
    else
        if (w_lam == -1._dl) then
            grhoa2=grhoa2 +grhov*a2**2
!print *, 'dsd', a, grhov*a2**2/grhom
        else
    ...
    if (quintessence .or. w_lam /= -1 .and. w_Perturb) then
        EV%w_ix = neq+1
        neq=neq+2
        maxeq=maxeq+2
    else
    ...
    if (quintessence) then
        call Quint_ValsAta(a,phi,phidot,Voip)
        grhoex_t=phidot**2/2 + a2*Voip*Vofphi(phi,0)
        grho=grho+grhoex_t
    ...
    real(dl) phidot, phi,grhoex_t, Voip
    !non-flat vars
    ...
    if (EV%TightCoupling) then
        !  ddota/a
        if (quintessence) then
            gpres = gpres + (grhog_t+grhor_t)/3 - grhoex_t+phidot**2
        else
