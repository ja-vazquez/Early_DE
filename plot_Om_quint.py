...
names = ['Om_Quint_B1','Om_Quint_B2','Om_Quint_B3','Om_Quint_B4','Om_Quint_B5']
rs = ['135.47','140.47','143.12','144.72','146.90']
rs_lcdm = 149.01
s=0
inter_Oede = []
lnat, Omt, Oedet, Oedet0 = [], [], [], []
for name in names:
...
namesq = ['Quint_B1','Quint_B2','Quint_B3','Quint_B4','Quint_B5']
lnaat, Dat, Dht = [], [], []


pnames=open(dir+'test_Quint_LCDM'+'.dat').readlines()
lpnames = len(pnames)
Da_lcdm, Dh_lcdm = [], []
...
if True:
 fig =pylab.figure(figsize=(15,18))
 ...
 for i in range(0,len(namesq)):
    ax6.plot(lnaat[i],np.array(Dht[i])*(rs_lcdm/float(rs[i])),color=colour(i+1))
